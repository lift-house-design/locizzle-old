		...
		if($doublecheck > 0) {
			while($row1 = mysql_fetch_assoc($query1)) {
				...
				echo 	$street.' '.$city.', '.$state.', '.$zip.
					 	'&nbsp;&nbsp;|&nbsp;<a style="color:#60BDB8;" href="./pending-operations.php?resend='.$quoteRequest_id.'">Resend</a> | <a style="color: #F2713C;" href="pending-operations.php?delete='.$quoteRequest_id.'">Delete</a> |<span class="insp_move_links" style="float:right;">Needed by: '.$date.'</span>
					  </h3>
				...
				if($hasBids)
				{
					...
					while($row2=mysql_fetch_assoc($query2))
					{
						...
						echo 	'<b class="bold">Inspector: </b><a target="_blank" href="./inspector.php?id='.$insp_id.'">'.$insp_first_name.' '.$insp_last_name.'</a>
								<span class="test" style=" float:right; height:10px;"><input type="hidden" class="hidden" name="accept_quote" value="'.$insp_id.'" /><input class="accept_quote_button" type="submit" value="+ Accept Quote" name="accept" /></span>
								</br>
								<b class="bold">Quote Amount: </b>$'.$bid_amount.'</br></br><hr /></br>';
					}
