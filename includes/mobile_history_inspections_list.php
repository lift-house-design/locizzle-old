<?php
	...
	$query1=mysql_query("SELECT * from property, person, bid, quoteRequest where quoteRequest.person='$broker_id' AND bid.status='accepted' AND quoteRequest.status='accepted' AND property.id=quoteRequest.property AND bid.quoteRequest=quoteRequest.id AND inspector=person.id");
				...
				$insp_last_name=$row1['last_name'];
						...
						if ($quoteRequest_id != $group) {
							...
							echo '</div><h3 class="inspector_list">
								...
								<b class="bold">Accepted Quote: </b>$'.$bid_amount.'</br></br><hr /></br>';
