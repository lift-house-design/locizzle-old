<?php
	...
	$query1=mysql_query("SELECT * from property, person, bid, quoteRequest where quoteRequest.person='$broker_id' AND bid.status='paid' AND quoteRequest.status='accepted' AND property.id=quoteRequest.property AND bid.quoteRequest=quoteRequest.id AND inspector=person.id");
